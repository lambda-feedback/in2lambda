...
def frozen(draft: str | Path) -> tuple[dict[str, Any], str]:
    """A draft and the markdown it was written from, still unmoved.
    ...
    path = Path(draft)
    found = _draft(path)
    raw, markdown = _source(path.parent / found["source"])
    if _digest(raw) != found["hash"]:
        raise DraftExists(
            f"{found['source']} has changed since {path.name} was written from it, so "
...
def add(file: str, start_over: bool = False) -> Path:
    ...
    written to ``FILE.draft.json``, so that whatever quotes the source by line range can
    ...
    Raises:
        ...
        DraftUnreadable: there is a draft beside the file that nothing here wrote,
    ...
    if not start_over:
        if draft.is_file():
            ...
            if existing["hash"] != digest:
                raise DraftExists(
                    f"{source.name} has changed since {draft.name} was written from "
                    "it. Run in2lambda source add --start-over to freeze it again, "
                    "which invalidates every line range taken from the old draft."
                )
...
def show(draft: str | Path) -> str:
    ...
    found, markdown = frozen(draft)

    ids = {block["start"]: block["id"] for block in found["blocks"]}
