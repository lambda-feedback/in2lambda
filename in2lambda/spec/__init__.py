    ...
    def matches(
        ...
        Args:
            elements: Every block of the document, each beside the panflute element it
                is. A ``depth`` constraint reads the block; every other constraint, and
                every predicate, reads the element.
...
def _roles(
    ...
    A block whose children hold a role holds no role itself. A block spans its children,
    so a question quoted from the whole of a list item and a part quoted from an item
    nested inside that list item would be two fields written over the same lines, which
    :func:`in2lambda.draft.record` refuses. The spec quotes the item's own paragraph into
    the question and the nested items into the parts, and one selector may match both.
    """
    found = [
        next(
            (
