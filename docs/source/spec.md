...
saying that the field - `q2.solution`, say - is already written and that no command fills a field
twice. `in2lambda draft field replace` changes the wording of one, `in2lambda draft field set`
quotes other lines of the source into one, and `in2lambda source add --start-over` begins the
draft again.
